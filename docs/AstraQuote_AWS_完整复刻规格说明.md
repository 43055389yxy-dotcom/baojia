...
AI 结果不是最终事实：需求解析阶段必须用 schema、类型、单位、允许字段和逐字证据校验，再封存为统一事实表。事实表封存后，缓存、客户确认、产品匹配、型号选择和最终报价只能读取这张表，禁止再次从不可变原话恢复、猜测或覆盖字段。不可变原话仅用于页面对照、审计日志和证明既有字段证据；旧草稿确需迁移时必须显式升级事实表版本，不能在正常报价流程中偷偷重解析。

解析阶段若发现字段缺失或证据不一致，必须把“当前组件归属原文 + 固定字段模板 + 校验错误”交回同一组件 AI 修复；程序不得维护“写入、摄取、导入、流入”等不断增长的业务同义词表，也不得自行决定这些词对应哪个计价字段。程序只验证 AI 返回字段是否属于模板、类型和单位是否正确、证据是否逐字存在、同一事实是否恰好归属一次。
