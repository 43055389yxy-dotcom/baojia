on run
	...
	try
		...
		set encodedPassword to my replaceText("@", "%40", vncPassword)
		open location "vnc://:" & encodedPassword & "@localhost:" & localPort
	on error errorMessage
		display alert "无法读取远程桌面密码" message errorMessage as critical
	end try
end run
...
on replaceText(findText, replacementText, sourceText)
	...
	set replacedText to sourceParts as text
