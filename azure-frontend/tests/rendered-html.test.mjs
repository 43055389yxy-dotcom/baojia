...
test("server-renders the AstraQuote product shell", async () => {
  ...
  assert.match(html, /Microsoft Azure 智能报价/);
...
test("client uses the live quote job API and keeps official-source copy", async () => {
  ...
  assert.doesNotMatch(page, />AWS 报价</);
  ...
  assert.doesNotMatch(page, /astraquote\.aws\.current-sales-region\.v2/);
  ...
  assert.match(layout, /Microsoft Azure 智能报价/);
