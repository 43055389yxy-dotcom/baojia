...
const EXPECTED_CLOUD_PROVIDER = "azure" as const;
...
export default function CustomerConfirmationPage() {
  ...
  const [addingConfigurationInProgress, setAddingConfigurationInProgress] = useState(() => {
    ...
    return window.sessionStorage.getItem(`astraquote:azure:addition:${token}`) === "active";
  });
  const [regionConfirmationInProgress, setRegionConfirmationInProgress] = useState(() => {
    if (typeof window === "undefined" || !token) return false;
    return window.sessionStorage.getItem(`astraquote:azure:region-confirmation:${token}`) === "active";
  });
    ...
    fetch(`${API_BASE}/api/confirmation-sessions/${token}`, { cache: "no-store" })
      .then(async (response) => {
        ...
        if (payload.cloud_provider !== EXPECTED_CLOUD_PROVIDER) {
          throw new Error("该链接不属于 Microsoft Azure 报价系统，系统已阻止跨云读取。");
        }
      ...
      .then((payload) => {
        ...
        if (
          ...
          window.sessionStorage.setItem(`astraquote:azure:region-confirmation:${token}`, "active");
        }
        if (!["reviewing", "submitted", "processing"].includes(payload.status)) {
          setRegionConfirmationInProgress(false);
          window.sessionStorage.removeItem(`astraquote:azure:region-confirmation:${token}`);
        }
        if (["configuration_review", "approved", "completed"].includes(payload.status)) {
          setAddingConfigurationInProgress(false);
          window.sessionStorage.removeItem(`astraquote:azure:addition:${token}`);
          setRegionConfirmationInProgress(false);
          window.sessionStorage.removeItem(`astraquote:azure:region-confirmation:${token}`);
        }
      })
  ...
  async function submitConfigurationFeedback(
    ...
    if (addedConfiguration) {
      window.sessionStorage.setItem(`astraquote:azure:addition:${token}`, "active");
    }
    setReviewSeconds(0);
    if (componentId) {
