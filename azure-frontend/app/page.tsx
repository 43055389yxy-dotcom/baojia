...
function serviceActivityLogs(name: string): string[] {
  ...
  return ["识别 Microsoft Azure 产品身份", "锁定组件数据边界", "复核客户原始要求", "分析 Azure 托管服务覆盖范围", "比较功能适配程度", "解析自建部署拓扑", "恢复节点数量", "恢复处理器与内存", "规范化存储单位", "检查架构决策", "选择 Azure 官方目录路径", "安全报价路径准备完成"];
}
...
function activityLogStream(channel: ActivityChannel): string[] {
  ...
  const pricingLogs = isPricingChannel ? [
    "初始化独立报价上下文",
    "同步区域产品目录索引",
    "查询 Azure Retail Prices 服务目录",
    "扫描 Azure 产品属性与 SKU 约束",
    "校验区域与部署方式兼容性",
    "关联型号、规格和购买方式",
    "展开官方计费维度集合",
    "规范化月度用量与单位",
...
const CONFIRMATION_CONTEXT_KEY = "astraquote.azure.pending-confirmation.v1";
const QUOTE_JOB_CONTEXT_KEY = "astraquote.azure.pending-quote.v1";
const SALES_REGION_CONTEXT_KEY = "astraquote.azure.current-sales-region.v2";
...
export default function Home() {
  const [cloudProvider] = useState<CloudProvider>("azure");
  ...
  async function submitRequirement() {
    ...
    try {
      const endpoint = "/api/azure/quotes/region-preflight";
