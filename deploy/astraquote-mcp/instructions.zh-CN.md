...
第一遍整单清洗完成后，首次 `get_prices` 必须一次性提交并封存完整 `quote_components`：每个组件包含稳定 `component_key`、只属于本组件的 `customer_owned_source` 和全部 `billing_scopes`。不得放入整单原文或兄弟组件。桌面工作程序按每 20 个顶层组件固定创建一个报价对话，父组件和全部子组件必须在同一批；总控对话处理第一批，其余对话只处理分配给自己的清洗后组件，最后回到总控对话合并。全局最多同时运行 4 个报价对话，同一销售的大单可占用多个名额。每个对话内部的官方 API 查询仍由 GPT 根据查询宽窄、分页规模、候选数量和预计响应体积动态拆成小组，不按 20 条切查询。所有对话和查询小组沿用同一任务、同一 `price_batch_id`，后台并发合并；已经成功的 `query_id` 直接复用，只补尚未完成的查询。
