...
同一个机器进度状态最多自动续跑一次，计数必须跨进程重启保存。第一次停止后先读取后台组件状态，锁定并复用成功组件，只把尚未完成的组件重新组织成一次续跑任务。只有后台确认新增成功计费范围、完成组件或交付阶段首次向前推进，才算真实进展；换 query_id、增加失败次数、阶段来回切换、页面文字变化和仅更新时间都不算。补发一次后仍没有进展就停止当前对话并释放名额，其他组件批次继续。最终已有成功组件正常进入 `services` 和合计，未完成项全部进入 `unpriced_services` 并设置 `is_partial=true`，不得按 0 元、不得计入合计、不得整单报错丢弃成功项；Excel 必须把未完成项标为“未自动取得价格，请销售手动填写”。
